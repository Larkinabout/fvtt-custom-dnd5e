...
async function createRollRequest(config) {
  ...
  const chatData = {
    user: game.user.id,
    content: await foundry.applications.handlebars.renderTemplate("systems/dnd5e/templates/chat/roll-request-card.hbs", { buttons }),
...
/**
 * Parse roll request configuration.
 * @param {object} config The config
...
function parseRollRequest(config) {
  for ( let value of config.values ) {
    ...
    if ( slug in CONFIG.DND5E.enrichmentLookup.abilities ) {
      config.ability = CONFIG.DND5E.enrichmentLookup.abilities[slug]?.key || slug;
    } else if ( slug in CONFIG.DND5E.enrichmentLookup.skills ) {
      config.skill = CONFIG.DND5E.enrichmentLookup.skills[slug]?.key || slug;
    } else if ( slug in CONFIG.DND5E.enrichmentLookup.tools ) {
      config.tool = CONFIG.DND5E.enrichmentLookup.tools[slug]?.key || slug;
    } else if ( Number.isNumeric(value) ) {
      config.dc = Number(value);
...
/**
 * Parse attack or damage configuration.
 * @param {object} config The config
 * @returns {object} The parsed config
 */
