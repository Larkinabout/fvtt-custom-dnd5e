...
export class ItemInteractionsForm extends CustomDnd5eForm {
  ...
  async _prepareContext() {
    ...
    return {
      ...
      dropItemsImageShape: getSetting(DROP_ITEMS.IMAGE_SHAPE.KEY) || "none",
      dropItemsImageShapeChoices: DROP_ITEMS.IMAGE_SHAPE.CHOICES,
      dropItemsImageBorderColor: getSetting(DROP_ITEMS.IMAGE_BORDER_COLOR.KEY) || "#9f9275",
      dropItemsImageBorderThickness: getSetting(DROP_ITEMS.IMAGE_BORDER_THICKNESS.KEY) || 0,
  ...
  static async reset() {
    const reset = async () => {
      await Promise.all([
        ...
        resetSetting(DROP_ITEMS.IMAGE_SHAPE.KEY),
        resetSetting(DROP_ITEMS.IMAGE_BORDER_COLOR.KEY),
        resetSetting(DROP_ITEMS.IMAGE_BORDER_THICKNESS.KEY),
  ...
  static async submit(event, form, formData) {
    ...
    await Promise.all([
      ...
      setSetting(DROP_ITEMS.IMAGE_SHAPE.KEY,
        Object.keys(DROP_ITEMS.IMAGE_SHAPE.CHOICES).includes(data.dropItemsImageShape) ? data.dropItemsImageShape : "none"),
      setSetting(DROP_ITEMS.IMAGE_BORDER_COLOR.KEY, data.dropItemsImageBorderColor || "#9f9275"),
      setSetting(DROP_ITEMS.IMAGE_BORDER_THICKNESS.KEY, Math.max(0, Number(data.dropItemsImageBorderThickness) || 0)),
