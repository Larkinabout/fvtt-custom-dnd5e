...
import { getAdvantageMode, getSetting, registerSetting } from "./utils.js";

const constants = CONSTANTS.AUTOMATION.MOBS;
...
async function rollMobDamage(mobTokens, activity, usageConfig, dialogConfig, messageConfig) {
  ...
  const attackBonus = parseInt(dnd5e.dice.simplifyRollFormula(
  ...
  const rollNeeded = targetAc - attackBonus;
  const outOf = mobTokens.length;
  const successes = calculateSuccesses(advantageMode, rollNeeded, outOf) ?? 0;
 ...
 * @param {number} rollNeeded The minimum d20 roll needed to hit.
 * @param {number} outOf The number of attackers in the mob.
 * @returns {number|null} The number of successful hits.
 */
function calculateSuccesses(advantageMode, rollNeeded, outOf) {
  let probability;

  switch (advantageMode) {
    case "advantage":
      const failChance = (rollNeeded - 1) / 20;
      probability = 1 - (failChance * failChance);
      break;
    case "disadvantage":
      const successChance = (21 - rollNeeded) / 20;
      probability = successChance * successChance;
      break;
    default:
      probability = (21 - rollNeeded) / 20;
      break;
  }
